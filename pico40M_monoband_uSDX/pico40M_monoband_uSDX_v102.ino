/*
  pico 40M monoband radio V2.01 JR3XNW
...
#define inputPinI 26
#define inputPinQ 27
#define sampleRate 80000 //8000
#define pwmFrequency 40000  //20000
...
#define VOLUME_BUTTON_PIN 3       // Volume up button
//#define VOLUME_DOWN_BUTTON_PIN 4  // volume down button
volatile float volumeMultiplier = 1.0;     // Initial volume setting
...
void setup() {
  ...
  u8g2.begin();
  u8g2.setFlipMode(0);   // Set screen orientation, switch between 0 and 1(inversion)
  ...
  u8g2.drawStr(25, 20, "Waterfall v0.101");
...
void setup1() {
  ...
  pinMode(VOLUME_BUTTON_PIN, INPUT_PULLUP);       // Set volume up button pin as input
...
void loop1() {
  ...
  //float volumeMultiplier = 1.0; // Multiplier to increase volume. Adjust as needed.

  uint8_t pwmOutput = (uint8_t)(min((agcOutput + 1.0) * 127.5 * volumeMultiplier, 255.0));
  analogWrite(speakerPin, pwmOutput);
  /*
  analogWriteResolution(8);を変更する
  uint16_t pwmOutput = (uint16_t)(min((agcOutput + 1.0) * 2047.5 * volumeMultiplier, 4095.0));  //12ピットの場合
  analogWrite(speakerPin, pwmOutput);

  uint16_t pwmOutput = (uint16_t)(min((agcOutput + 1.0) * 32767.5 * volumeMultiplier, 65535.0));  //16ピットの場合
  analogWrite(speakerPin, pwmOutput);
  */
  delayMicroseconds(1000000 / sampleRate);  //100000

  // Check to see if the volume buttons have been pressed
  if (digitalRead(VOLUME_BUTTON_PIN) == LOW) {
    volumeMultiplier += 0.1;  // Increase volume by 0.1
    if (volumeMultiplier > 1.9) {
      volumeMultiplier = 1.0; // Reset to 1.0 after 1.9
    }
    // Debouncing process to prevent continuous button presses
    delay(200); 
  }
}